/*
 * sdp4430jet.c  --  SoC audio for TI OMAP4430 SDP
 *
...
#endif
#define JET_DIGITAL_NAME "Jet Speaker"//"FM Digital"
static int mcbsp_cfg;
static struct clk *abe_clk_for_dmic;
	...
	struct snd_pcm_hw_params *params)
{
	...
	if(be_id == OMAP_ABE_DAI_BT_VX) {
		ret = snd_soc_dai_set_fmt(cpu_dai,
			SND_SOC_DAIFMT_I2S |
			SND_SOC_DAIFMT_NB_NF |
			SND_SOC_DAIFMT_CBM_CFM);//SND_SOC_DAIFMT_DSP_B|SND_SOC_DAIFMT_NB_IF|SND_SOC_DAIFMT_CBM_CFM
	} else {
	ret = snd_soc_dai_set_fmt(cpu_dai,
			SND_SOC_DAIFMT_I2S |
			SND_SOC_DAIFMT_NB_NF |
	...
	if(be_id == OMAP_ABE_DAI_BT_VX) { 
		...
		/* Set McBSP clock to internal */
		switch (params_rate(params)) {
			case 44100: /* 44.117 */
			div = 17;//68;//20
			clk_id = OMAP_MCBSP_SYSCLK_CLKS_FCLK;
			freq = 25000000;//96000000;//
			break;
			case 48000: /* 48.032 */
			div = 16;//54;//
			clk_id = OMAP_MCBSP_SYSCLK_CLKS_FCLK;//OMAP_MCBSP_SYSCLK_CLK;
			freq = 25000000;//83000000;
			break;
			default:
		...
		ret = snd_soc_dai_set_sysclk(cpu_dai,clk_id ,
			freq,
			SND_SOC_CLOCK_IN);//64 * params_rate(params)
		if (ret < 0){
...
#else
	abe_clk_for_dmic= clk_get(NULL, "auxclk2_ck");
	if (IS_ERR(abe_clk_for_dmic)) {
		printk(KERN_ERR "Cannot request auxclk2\n");
		return -ENODEV;;
	}
	//set to 19.2 MHz
	clk_set_rate(abe_clk_for_dmic, 19200000);
	clk_enable(abe_clk_for_dmic);
	return 0;
#endif
}
...
#ifdef CONFIG_TWL6040_CODEC
	struct twl6040 *twl6040 = twl6040_codec->control_data;
	return twl6040_disable(twl6040);
#else
	clk_disable(abe_clk_for_dmic);
	clk_put(abe_clk_for_dmic);
#endif
}
...
#else
	return omap_abe_set_dl1_output(OMAP_ABE_DL1_NO_PDM);
#endif
}
...
{
	.name = JET_DIGITAL_NAME,
	.playback = {
		.stream_name = "FM Playback", //widget use this name, check omap-abe-dsp.c
		.channels_min = 1,
		.channels_max = 2,
 ...
 */
	...
	},
	{
		...
		/* FM */
		.codec_dai_name = JET_DIGITAL_NAME,
...
{
	int ret;
	printk(KERN_INFO "Jet SoC init\n");
	snd_soc_sdp4430.name = "jet";
	...
	if (ret < 0)
	{
		printk(KERN_ERR "%s register dai fail\n",__func__);
		goto err;
	}
	...
	if (ret){
		printk(KERN_ERR "%s platform data fail\n",__func__);
		goto err_dev;
	}
#ifdef CONFIG_TWL6040_CODEC
