...
{
	if ((cell->edv.percent == edv->percent) && (cell->edv.voltage==edv->voltage)
	&& cell->init)
		return;
 ...
 * working
 */
void fg_update_edv_flags(struct cell_state *cell, bool force_update)
{
	unsigned char soc;
	...
	} else if (((soc < cell->config->edv->edv[2].percent) || force_update) &&
	...
	} else if ((soc < cell->config->edv->edv[1].percent) || force_update) {
