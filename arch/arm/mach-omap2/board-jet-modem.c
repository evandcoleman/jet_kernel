/*
 * arch/arm/mach-omap2/board-jet-modem.c
 *
 ...
 */
static int jet_modem_switch(int new_state)
{
    int modem_pwrrst_ca;
    int state;
    int i;
...
static void jet_hsi_pad_conf(void)
{
    /*
...
static void jet_modem_pad_conf(void)
{
 ...
 */
void __init jet_modem_init(bool force_mux)
{
    int modem_enabled;
    ...
    if (unlikely(!gpio_is_valid(JET_MDM_PWR_EN_GPIO) ||
        gpio_request(JET_MDM_PWR_EN_GPIO, "MODEM VBAT") < 0)) {
        pr_err("Cannot control gpio %d\n", JET_MDM_PWR_EN_GPIO);
        goto err_pwr;
    }
    gpio_direction_output(JET_MDM_PWR_EN_GPIO, 1);
    ...
    modem_detected = jet_modem_switch(1);
    pr_info("Modem %sdetected\n", modem_detected ? "" : "NOT ");

    /* Disable Modem ONSW */
    modem_enabled = jet_modem_switch(0);
...
#endif
        /* Setup modem pad conf for jet platform */
        jet_modem_pad_conf();
        /* Allow HSI omap_device to be registered later */
        omap_hsi_allow_registration();

    } else {
        gpio_direction_output(JET_MDM_PWR_EN_GPIO, 0);
        gpio_free(JET_MDM_PWR_EN_GPIO);
    }
err_pwr:
    return;
}
...
static int late_jet_modem_init(void)
{
    int ret;
...
late_initcall(late_jet_modem_init);
